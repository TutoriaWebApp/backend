...
class SessaoViewSetTest(APITestCase):
	def setUp(self):
		self.usuario_aluno = UsuarioModel.objects.create_user(
			localizacao=Point(0, 0, srid=4326), email='aluno@example.com', password='testpassword123',
			nomePerfil='Aluno', cidade='Test City', estado='TS'
		)
		self.usuario_tutor = UsuarioModel.objects.create_user(
			localizacao=Point(0, 0, srid=4326), email='tutor@example.com', password='testpassword123',
	...
	def test_aceitar_solicitacao_recorrente(self):
		self.solicitacao.recorrente = True
		self.solicitacao.save()
		self.client.force_authenticate(user=self.usuario_tutor)
		url = reverse('aceitar-solicitacao-detail', args=[self.solicitacao.id])
		solicitacoes_antes = SolicitacaoModel.objects.count()
		
		response = self.client.patch(url)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		
		solicitacoes_depois = SolicitacaoModel.objects.count()
		self.assertEqual(solicitacoes_depois, solicitacoes_antes + 1)
from django.urls import reverse
from rest_framework import status
from rest_framework.test import APITestCase
from project.utils.GeoLocalizacaoUtil import Point
...
class SessaoViewSetTest(APITestCase):
	...
	def test_aceitar_solicitacao_recorrente(self):
		...
		nova_solicitacao = SolicitacaoModel.objects.last()
		self.assertEqual(nova_solicitacao.dataPretendida, self.solicitacao.dataPretendida + datetime.timedelta(days=7))
		self.assertTrue(nova_solicitacao.recorrente)
		self.assertEqual(nova_solicitacao.estado, SolicitacaoModel.EstadoSolicitacao.PENDENTE)

	def test_todas_solicitacoes_usuario(self):
		self.client.force_authenticate(user=self.usuario_aluno)
		url = reverse('todas-solicitacoes-list')
		response = self.client.get(url)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		self.assertEqual(len(response.data), 1)

	def test_todas_sessoes_usuario(self):
		self.client.force_authenticate(user=self.usuario_aluno)
		url = reverse('todas-sessoes-usuario-list')
		response = self.client.get(url)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		self.assertEqual(len(response.data), 1)

	def test_sessoes_tutor_verificacao(self):
		self.client.force_authenticate(user=self.usuario_tutor)
		url = reverse('sessoes-tutor-list')
		response = self.client.get(url, {'tutor_id': self.tutor.id})
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		self.assertEqual(len(response.data), 1)
